...
#define ORACLE_REVERSE_BUFFER_SIZE (8 * 1024 * 1024)  /* 8M entries per batch */
  ...
  while (fgets(line, sizeof(line), meta_fp)) {
    if (params->n_batches >= batch_capacity) {
      ...
      if (!new_positions || !new_sizes || !new_entries) {
        ...
        /* Keep originals if realloc failed (they're still valid) */
        ...
        break;  /* Stop reading, use what we have */
      }
...
static inline void oracleGeneralCompressedReverse_teardown(reader_t *reader) {
  if (reader->reader_params != NULL) {
    ...
    if (params->thread_running) {
      ...
      pthread_cond_signal(&params->cond);  /* Wake up thread */
      pthread_mutex_unlock(&params->mutex);
      
      pthread_join(params->decompress_thread, NULL);  /* Wait for thread to finish */
    }
