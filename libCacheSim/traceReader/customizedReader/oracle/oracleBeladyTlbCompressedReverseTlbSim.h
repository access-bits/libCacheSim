...
/*
 * Oracle Belady TLB compressed binary trace format - REVERSE chronological order
 * WITH TLB simulation fields passed to the algorithm.
 *
 * Passes tlb_miss, cpu, and next_access_timestamp to the request so the
 * cache algorithm can perform Belady TLB simulation.
 *
 * Entry format (after decompression, within each batch in reverse order):
 *   struct {
 *     uint64_t vaddr;
 *     int64_t  next_access_timestamp;   // next access time for Belady TLB eviction
 *     uint8_t  tlb_miss;
 *     uint8_t  cpu;
 *     int64_t  page_next_access_time;   // next TLB-miss vtime for Belady cache eviction
 *   };
 */
...
#define ORACLE_BELADY_TLB_REVERSE_TLB_SIM_BUFFER_SIZE (24 * 1024 * 1024)

typedef struct {
  uint64_t vaddr;
  int64_t  next_access_timestamp;
  uint8_t  tlb_miss;
  uint8_t  cpu;
  int64_t  page_next_access_time;
} __attribute__((packed)) oracle_belady_tlb_reverse_tlb_sim_entry_t;

typedef struct {
  oracle_belady_tlb_reverse_tlb_sim_entry_t *entry_buffer[2];
  char *compressed_buf;
  ...
  FILE *file_handle;
  bool eof;
} oracle_belady_tlb_reverse_tlb_sim_params_t;

static void* oracleBeladyTlbCompressedReverseTlbSim_decompress_worker(void *arg) {
  oracle_belady_tlb_reverse_tlb_sim_params_t *params =
    (oracle_belady_tlb_reverse_tlb_sim_params_t *)arg;
      ...
      if (fseek(params->file_handle, batch_pos, SEEK_SET) == 0) {
        ...
        if (fread(params->compressed_buf, 1, compressed_size, params->file_handle)
          ...
          size_t decompressed_size = ZSTD_decompress(
            ...
            params->buffer_capacity * sizeof(oracle_belady_tlb_reverse_tlb_sim_entry_t),
            params->compressed_buf,
            compressed_size);
          if (!ZSTD_isError(decompressed_size)) {
            pthread_mutex_lock(&params->mutex);
            params->buffer_size[buffer_idx] =
              decompressed_size / sizeof(oracle_belady_tlb_reverse_tlb_sim_entry_t);
            params->buffer_pos[buffer_idx] =
              (ssize_t)params->buffer_size[buffer_idx] - 1;
...
static inline void request_decompress_belady_tlb_sim(
    oracle_belady_tlb_reverse_tlb_sim_params_t *params,
...
static inline bool switch_buffer_belady_tlb_sim(
    oracle_belady_tlb_reverse_tlb_sim_params_t *params) {
  ...
  if (params->n_batches > 0) {
    ...
    if (pct / 5 != last_pct / 5) {
      INFO("oracleBeladyTlbCompressedReverseTlbSim: %zu/%zu batches read (%zu%%)\n",
  ...
  if (params->current_batch >= 0) {
    ...
    request_decompress_belady_tlb_sim(params, inactive_buffer, params->current_batch);
  } else {
    params->eof = true;
  }
  return true;
...
static inline int oracleBeladyTlbCompressedReverseTlbSim_setup(reader_t *reader) {
  reader->trace_type = ORACLE_BELADY_TLB_COMPRESSED_REVERSE_TLB_SIM_TRACE;
  reader->trace_format = BINARY_TRACE_FORMAT;
  reader->item_size = sizeof(oracle_belady_tlb_reverse_tlb_sim_entry_t);
  reader->obj_id_is_num = true;
  reader->trace_start_offset = 0;

  oracle_belady_tlb_reverse_tlb_sim_params_t *params =
    (oracle_belady_tlb_reverse_tlb_sim_params_t *)malloc(
      sizeof(oracle_belady_tlb_reverse_tlb_sim_params_t));

  params->buffer_capacity = ORACLE_BELADY_TLB_REVERSE_TLB_SIM_BUFFER_SIZE;
  params->entry_buffer[0] = (oracle_belady_tlb_reverse_tlb_sim_entry_t *)malloc(
    sizeof(oracle_belady_tlb_reverse_tlb_sim_entry_t) * params->buffer_capacity);
  params->entry_buffer[1] = (oracle_belady_tlb_reverse_tlb_sim_entry_t *)malloc(
    sizeof(oracle_belady_tlb_reverse_tlb_sim_entry_t) * params->buffer_capacity);
  ...
  params->compressed_buf_capacity = ZSTD_compressBound(
    params->buffer_capacity * sizeof(oracle_belady_tlb_reverse_tlb_sim_entry_t)) + 1024;
  ...
  if (pthread_create(&params->decompress_thread, NULL,
                     oracleBeladyTlbCompressedReverseTlbSim_decompress_worker,
                     params) != 0) {
  ...
  INFO("Setup Belady TLB TlbSim reverse oracle reader: %zu batches, entry_size=%zu\n",
       params->n_batches, sizeof(oracle_belady_tlb_reverse_tlb_sim_entry_t));

  return 0;
}

static inline void oracleBeladyTlbCompressedReverseTlbSim_teardown(reader_t *reader) {
  if (reader->reader_params != NULL) {
    oracle_belady_tlb_reverse_tlb_sim_params_t *params =
      (oracle_belady_tlb_reverse_tlb_sim_params_t *)reader->reader_params;
...
static inline int oracleBeladyTlbCompressedReverseTlbSim_read_one_req(
    reader_t *reader, request_t *req) {
  oracle_belady_tlb_reverse_tlb_sim_params_t *params =
    (oracle_belady_tlb_reverse_tlb_sim_params_t *)reader->reader_params;
  ...
  if (params->buffer_pos[active] < 0) {
    ...
    if (params->buffer_size[0] == 0 && params->buffer_size[1] == 0) {
      ...
      request_decompress_belady_tlb_sim(params, active, params->current_batch);
      ...
      if (params->current_batch >= 0) {
        ...
        request_decompress_belady_tlb_sim(params, next_buffer,
                                          params->current_batch);
      } else {
        params->eof = true;
      }
    } else {
      if (!switch_buffer_belady_tlb_sim(params)) {
        req->valid = FALSE;
        return 1;
      }
  ...
  oracle_belady_tlb_reverse_tlb_sim_entry_t *entry =
  ...
  req->next_access_ts = entry->next_access_timestamp;
