    ...
    if (batch_idx >= 0 && batch_idx < (ssize_t)params->n_batches) {
      ...
      if (fseek(params->file_handle, batch_pos, SEEK_SET) == 0) {
        ...
        if (compressed_size > params->compressed_buf_capacity) {
          ...
          if (!new_buf) {
            ERROR("mergedTrace: failed to realloc compressed buffer to %zu\n",
                  params->compressed_buf_capacity);
            goto done;
          }
        ...
        if (fread(params->compressed_buf, 1, compressed_size,
          ...
          if (expected_entries > params->buffer_capacity) {
            ...
            if (!new_buf) {
              ERROR("mergedTrace: failed to realloc entry buffer\n");
              goto done;
            }
          ...
          if (!ZSTD_isError(dec)) {
            ...
            if (got_entries != expected_entries) {
              WARN("mergedTrace: batch %zd entry count mismatch: "
                   "expected %zu, got %zu\n",
            ...
            ERROR("mergedTrace: zstd error batch %zd: %s\n", batch_idx,
...
static inline bool mergedTrace_switch_buffer(merged_trace_params_t *params) {
  ...
  if (params->n_batches > 0) {
    ...
    if (pct / 5 != last_pct / 5) {
      INFO("mergedTrace: %zu/%zu batches read (%zu%%)\n",
...
static inline int mergedTrace_setup(reader_t *reader) {
  ...
  if (!params->entry_buffer[0] || !params->entry_buffer[1]) {
    ERROR("mergedTrace: failed to allocate entry buffers\n");
  ...
  if (!params->compressed_buf) {
    ERROR("mergedTrace: failed to allocate compressed buffer\n");
    free(params->entry_buffer[0]);
    free(params->entry_buffer[1]);
    free(params);
    return 1;
  ...
  if (!params->file_handle) {
    ERROR("mergedTrace: failed to open %s for bg thread\n",
          reader->trace_path);
    free(params->entry_buffer[0]);
    free(params->entry_buffer[1]);
  ...
  if (!meta_fp) {
    ERROR("mergedTrace: failed to open .meta file: %s\n", meta_path);
  ...
  INFO("mergedTrace: reading metadata from %s\n", meta_path);
  ...
  while (fgets(line, sizeof(line), meta_fp)) {
    if (strncmp(line, "# address_mode:", 15) == 0) {
      if (strstr(line + 15, "virtual_addresses") != NULL) {
        ...
        INFO("mergedTrace: address_mode=virtual_addresses, "
             "will convert to pages (>> 12)\n");
      } else {
        INFO("mergedTrace: address_mode=pages\n");
      }
  ...
  if (!params->batch_positions || !params->batch_sizes ||
      !params->batch_entries) {
    ERROR("mergedTrace: failed to allocate batch info arrays\n");
    fclose(meta_fp);
    fclose(params->file_handle);
    free(params->entry_buffer[0]);
    free(params->entry_buffer[1]);
  ...
  while (fgets(line, sizeof(line), meta_fp)) {
    if (params->n_batches >= batch_capacity) {
      ...
      if (!params->batch_positions || !params->batch_sizes ||
          !params->batch_entries) {
        ERROR("mergedTrace: failed to grow batch arrays\n");
        break;
      }
    }
  ...
  if (params->n_batches == 0) {
    ERROR("mergedTrace: no batch information in .meta file\n");
    fclose(params->file_handle);
    free(params->entry_buffer[0]);
    free(params->entry_buffer[1]);
    free(params->compressed_buf);
  ...
  INFO("mergedTrace: %zu batches, %lu total entries\n", params->n_batches,
  ...
  if (pthread_create(&params->decompress_thread, NULL,
    ...
    ERROR("mergedTrace: failed to create decompression thread\n");
    fclose(params->file_handle);
    free(params->entry_buffer[0]);
    free(params->entry_buffer[1]);
    free(params->compressed_buf);
  ...
  INFO("mergedTrace: setup complete, background decompression enabled\n");
